...
const Inventory = () => {
    ...
    const [isViewTransModalOpen, setIsViewTransModalOpen] = useState(false);
    const [viewTransData, setViewTransData] = useState(null);
    ...
    const addMultiRow = () => {
        if (multiItemForm.length < 20) setMultiItemForm([...multiItemForm, { ...initSingleItem, category: activeTab === 'Kho Nông sản' ? 'Nông sản đầu ra' : 'Vật tư đầu vào' }]);
        else alert("Chỉ được thêm tối đa 20 mặt hàng!");
    };

    const removeMultiRow = (index) => {
        if (multiItemForm.length > 1) setMultiItemForm(multiItemForm.filter((_, i) => i !== index));
    };
    ...
    const initTransForm = { 
        ...
        paymentStatus: 'Đã thanh toán', paymentMethod: 'Tiền mặt', updateBasePrice: true, vatRate: 0,
    ...
    const fetchData = React.useCallback(async () => {
        try {
            const [itemsRes, transRes, membersRes] = await Promise.all([
                getAllItems(), getTransactions(activeTab === 'Đã nhập' ? 'Nhập' : activeTab === 'Đã xuất' ? 'Xuất' : ''), getAllMembers()
            ]);
            setItems(itemsRes.data); setMembersList(membersRes.data);
    ...
    const handleItemSubmit = async (e) => {
        ...
        } catch (error) { alert("Lỗi lưu hàng hóa! " + (error.response?.data?.message || "")); }
    };
    ...
    const handleTransSubmit = async (e) => {
        ...
        try {
            ...
            alert(error.response?.data?.message || "Lỗi tạo phiếu! Vui lòng kiểm tra lại số lượng hoặc Tồn Quỹ."); 
        }
    };
    
    const handleDeleteTrans = async (id) => {
        if (window.confirm("Xóa phiếu này hệ thống sẽ tự động hoàn lại Số lượng Kho (và Tiền Sổ Quỹ / Công nợ nếu có). Bạn có chắc?")) {
    ...
    // Tính toán có Thuế VAT
    const getTransCalculations = () => {
        const subTotal = transForm.products.reduce((sum, item) => sum + (Number(item.quantity) * Number(item.unitPrice)), 0);
        const vatValue = subTotal * (Number(transForm.vatRate || 0) / 100);
        const totalAmount = subTotal + vatValue;
        return { subTotal, vatValue, totalAmount };
    };
    const calc = getTransCalculations();
        ...
        <div className="page-wrapper" style={{ padding: '20px', height: '100%', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
            <style>{`
                * { box-sizing: border-box; }
                ...
                .bulk-row { display: grid; grid-template-columns: 2fr 1.3fr 1fr 1fr 1.2fr 1.2fr 1.2fr 1.5fr 40px; gap: 12px; align-items: center; margin-bottom: 10px; padding: 10px 15px; background: #f8f9fa; border-radius: 6px; border: 1px solid #e0e0e0; width: 100%; }
                .bulk-header { display: grid; grid-template-columns: 2fr 1.3fr 1fr 1fr 1.2fr 1.2fr 1.2fr 1.5fr 40px; gap: 12px; font-weight: bold; color: #2c3e50; margin-bottom: 10px; padding: 0 15px; width: 100%; }
                .read-only-field { background: #f1f2f6; padding: 10px; border-radius: 4px; border: 1px solid #dcdde1; color: #2c3e50; font-weight: bold; margin-top: 5px; }
                ...
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    {canManageCategory && (
                        <button className="btn btn-outline" onClick={() => { 
                            ...
                            setEditingId(null); setIsItemModalOpen(true); 
                        }}>➕ Thêm danh mục</button>
                    )}
                    {canCreateReceipt && (
                        <>
                            <button className="btn btn-success" onClick={() => { 
                                setTransForm({ ...initTransForm, type: 'Nhập', reason: 'Nhập mới', paymentStatus: 'Đã thanh toán', paymentMethod: 'Tiền mặt', updateBasePrice: true, vatRate: 0 }); setIsTransModalOpen(true); 
                            }}>📥 Lập Phiếu Nhập</button>
                            <button className="btn btn-danger" onClick={() => { 
                                setTransForm({ ...initTransForm, type: 'Xuất', reason: 'Bán hàng', paymentMethod: 'Tiền mặt', isCredit: false, vatRate: 0 }); setIsTransModalOpen(true); 
                ...
                <>
                    <div className="toolbar">
                        ...
                        <select className="tool-select" style={{width: 'auto'}} value={sortItemName} onChange={(e) => setSortItemName(e.target.value)}>
                            <option value="">Sắp xếp Tên: Mặc định</option><option value="ASC">Từ A - Z</option><option value="DESC">Từ Z - A</option>
                        </select>
                    </div>
                    <div className="card-container">
                        <div className="table-scroll">
                            <table>
                                <colgroup>
                                    <col style={{ width: '15%' }} />{activeTab === 'Kho Nông sản' && <col style={{ width: '10%' }} />}
                                ...
                                <thead>
                                    <tr>
                                        <th>Tên mặt hàng</th>{activeTab === 'Kho Nông sản' && <th>Chất lượng</th>}
                                        <th>Phân loại</th><th>Số lượng</th><th>Đơn vị</th><th>Lô hàng</th><th>Hạn dùng</th>
                                        <th>Đơn giá gốc</th><th>Hành động</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {filteredItems.map(item => {
                                        ...
                                        <tr key={item.id}>
                                            ...
                                            <td>
                                                <button onClick={() => { setViewItemData(item); setIsViewItemModalOpen(true); }} style={{border: 'none', background: 'none', cursor:'pointer', marginRight: '5px'}} title="Xem chi tiết">👁️</button>
                                                {canManageCategory && (
                                                    <>
                                                        <button onClick={() => { setMultiItemForm([{...item}]); setEditingId(item.id); setIsItemModalOpen(true); }} style={{border: 'none', background: 'none', cursor:'pointer', marginRight: '5px'}} title="Sửa">✏️</button>
                                                        <button onClick={() => handleDeleteItem(item.id)} style={{border: 'none', background: 'none', cursor:'pointer'}} title="Xóa">🗑️</button>
                                                    </>
                                                )}
                                            </td>
                                        </tr>
                    ...
                    <div className="card-container">
                        <div className="table-scroll">
                            <table>
                                <thead>
                                    <tr>
                                        <th>Ngày & Mã Phiếu</th><th>Sản phẩm</th><th>Thanh toán & Thuế</th><th>Số lượng</th>
                                ...
                                <tbody>
                                    {filteredTrans.map(t => (
                                        <tr key={t.id}>
                                            ...
                                            <td><b>{t.Inventory?.itemName}</b><br/><small style={{color: '#7f8c8d'}}>{t.Inventory?.category}</small></td>
                                            <td>
                                                <b style={{color: '#2980b9'}}>{new Intl.NumberFormat('vi-VN').format(t.totalAmount || 0)} đ</b><br/>
                                                <small style={{color: '#e74c3c'}}>VAT: {t.vatRate || 0}%</small>
                                            </td>
                                            ...
                                            <td>
                                                {activeTab === 'Đã nhập' ? t.supplier : (
                                                    <>
                                                        ...
                                                        {t.isCredit && <span style={{background: '#e74c3c', color: 'white', padding: '2px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold'}}>GHI NỢ</span>}
                                                    </>
                                                )}
                                            </td>
                                            <td>
                                                <button onClick={() => { setViewTransData(t); setIsViewTransModalOpen(true); }} style={{border: 'none', background: 'none', cursor:'pointer', marginRight: '5px'}} title="Xem chi tiết Phiếu">👁️</button>
                                                {canDeleteReceipt && <button onClick={() => handleDeleteTrans(t.id)} style={{border: 'none', background: 'none', cursor:'pointer'}} title="Xóa Phiếu">🗑️</button>}
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                ...
                <div className="modal-overlay">
                    <div className="modal-content" style={{ width: '95%', maxWidth: editingId ? '500px' : '1300px' }}>
                        <div className="modal-header" style={{ padding: '15px 20px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <button onClick={() => setIsItemModalOpen(false)} style={{border: 'none', background: 'none', fontSize: '20px', cursor: 'pointer', color: '#999'}} >✕</button>
                        </div>
                        <form id="itemForm" onSubmit={handleItemSubmit} className="modal-body">
                            {editingId ? (
                                <>
                                    <label style={{display: 'block', marginBottom: '15px'}}><b>Tên mặt hàng:</b><input className="tool-input" style={{marginTop: '5px'}} value={multiItemForm[0].itemName} onChange={e => handleMultiChange(0, 'itemName', e.target.value)} required/></label>
                                    <label style={{display: 'block', marginBottom: '15px'}}><b>Phân loại:</b>
                                        <select className="tool-select" style={{marginTop: '5px'}} value={multiItemForm[0].category} onChange={e => handleMultiChange(0, 'category', e.target.value)}>
                                            <option value="Vật tư đầu vào">Vật tư đầu vào</option><option value="Công cụ dụng cụ">Công cụ dụng cụ</option><option value="Nông sản đầu ra">Nông sản đầu ra</option>
                                        </select>
                                    </label>
                                    <label style={{display: 'block', marginBottom: '15px'}}><b>Phân loại chất lượng:</b>
                                        <select className="tool-select" style={{marginTop: '5px', opacity: multiItemForm[0].category !== 'Nông sản đầu ra' ? 0.3 : 1, border: multiItemForm[0].category === 'Nông sản đầu ra' ? '2px solid #8e44ad' : ''}} value={multiItemForm[0].quality || 'Tiêu chuẩn'} onChange={e => handleMultiChange(0, 'quality', e.target.value)} disabled={multiItemForm[0].category !== 'Nông sản đầu ra'}>
                                            <option value="Tiêu chuẩn">Tiêu chuẩn</option><option value="Loại 1">Loại 1</option><option value="Loại 2">Loại 2</option><option value="Loại 3">Loại 3</option><option value="Hàng dạt">Hàng dạt</option>
                                        </select>
                                    </label>
                                    ...
                                    <label style={{display: 'block', marginBottom: '15px'}}><b>Số Lô hàng:</b><input className="tool-input" style={{marginTop: '5px'}} value={multiItemForm[0].batchNumber || ''} onChange={e => handleMultiChange(0, 'batchNumber', e.target.value)}/></label>
                                    <label style={{display: 'block', marginBottom: '15px'}}><b>Hạn sử dụng:</b><input className="tool-input" type="date" style={{marginTop: '5px'}} value={multiItemForm[0].expiryDate || ''} onChange={e => handleMultiChange(0, 'expiryDate', e.target.value)}/></label>
                                    <label style={{display: 'block', marginBottom: '15px'}}><b>Nhà cung cấp:</b><input className="tool-input" style={{marginTop: '5px'}} value={multiItemForm[0].supplier || ''} onChange={e => handleMultiChange(0, 'supplier', e.target.value)}/></label>
                                    <label style={{display: 'block', marginBottom: '10px'}}><b>Đơn giá gốc (VNĐ):</b><input className="tool-input" type="number" style={{marginTop: '5px'}} value={multiItemForm[0].unitPrice || ''} onChange={e => handleMultiChange(0, 'unitPrice', e.target.value)}/></label>
                                </>
                            ) : (
                                <div style={{overflowX: 'auto', paddingBottom: '10px'}}>
                                    <div style={{minWidth: '1100px', width: '100%'}}>
                                        <div className="bulk-header">
                                            <div>Tên mặt hàng (*)</div><div>Phân loại</div><div>Chất lượng</div><div>Đơn vị (*)</div><div>Số lô</div><div>Hạn sử dụng</div><div>Đơn giá (VNĐ)</div><div>Nhà cung cấp</div><div></div>
                                        </div>
                                        {multiItemForm.map((item, index) => (
                                            <div className="bulk-row" key={index}>
                                                ...
                                                <button type="button" onClick={() => removeMultiRow(index)} style={{background: '#ffeaa7', border: '1px solid #fdcb6e', color: '#d63031', cursor: 'pointer', fontSize: '16px', width: '100%', height: '100%', minHeight: '38px', borderRadius: '4px', fontWeight: 'bold'}}>✕</button>
                                            </div>
                                        ))}
                                    </div>
                                    {multiItemForm.length < 20 && (
                                        <button type="button" className="btn btn-outline" style={{borderStyle: 'dashed', color: '#27ae60', borderColor: '#27ae60', width: '100%', marginTop: '10px'}} onClick={addMultiRow}>+ Thêm một dòng (Đang có: {multiItemForm.length}/20)</button>
                                    )}
                                </div>
                            )}
                        </form>
            ...
            {/* MODAL 2: FORM LẬP PHIẾU NHẬP/XUẤT (CÓ VAT) */}
            {isTransModalOpen && (
                <div className="modal-overlay">
                    <div className="modal-content" style={{ maxWidth: '800px' }}>
                        <div className="modal-header" style={{padding: '15px 20px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                            <h3 style={{margin: 0, color: transForm.type === 'Nhập' ? '#27ae60' : '#e74c3c'}}>📝 LẬP PHIẾU {transForm.type.toUpperCase()} KHO</h3>
                            <button style={{border: 'none', background: 'none', fontSize: '20px', cursor: 'pointer', color: '#999'}} onClick={() => setIsTransModalOpen(false)}>✕</button>
                        </div>
                        <form id="transForm" onSubmit={handleTransSubmit} className="modal-body">
                            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '25px', background: '#f8f9fa', padding: '15px', borderRadius: '8px', border: '1px solid #e0e0e0'}}>
                                ...
                                {transForm.type === 'Nhập' ? (
                                    <>
                                        ...
                                        <div style={{gridColumn: 'span 2', background: '#ecf0f1', padding: '10px', borderRadius: '6px', marginTop: '10px', border: '1px dashed #bdc3c7'}}>
                                            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '10px'}}>
                                                <label><b>Thuế VAT (%):</b>
                                                    <input type="number" step="0.1" min="0" className="tool-input" style={{marginTop: '5px'}} value={transForm.vatRate} onChange={e => setTransForm({...transForm, vatRate: e.target.value})} placeholder="Vd: 8" />
                                                </label>
                                                <label><b>Thanh toán NCC:</b>
                                                    <select className="tool-select" style={{marginTop: '5px', fontWeight: 'bold'}} value={transForm.paymentStatus} onChange={e => setTransForm({...transForm, paymentStatus: e.target.value})}>
                                                        <option value="Đã thanh toán">Trừ tiền Sổ Quỹ</option>
                                                        <option value="Ghi nợ NCC">Ghi nợ NCC</option>
                                                    </select>
                                                </label>
                                            </div>
                                            
                                            {transForm.paymentStatus === 'Đã thanh toán' && (
                                                <label style={{display:'block', marginBottom: '10px'}}><b>Hình thức Trừ Quỹ:</b>
                                                    <select className="tool-select" style={{marginTop: '5px'}} value={transForm.paymentMethod} onChange={e => setTransForm({...transForm, paymentMethod: e.target.value})}>
                                                        <option value="Tiền mặt">Tiền mặt</option><option value="Chuyển khoản">Chuyển khoản</option>
                                                    </select>
                                                </label>
                                            )}

                                            <label style={{display: 'flex', alignItems: 'center', cursor: 'pointer', background: '#eaf2f8', padding: '8px', borderRadius: '4px'}}>
                                                <input type="checkbox" style={{width: '20px', height: '20px', marginRight: '10px'}} checked={transForm.updateBasePrice} onChange={e => setTransForm({...transForm, updateBasePrice: e.target.checked})} />
                                                <b style={{color: '#2980b9'}}>Tự động cập nhật Giá gốc Niêm yết mới</b>
                                            </label>
                                        </div>
                                    </>
                                ) : (
                                    <>
                                        <div><b style={{color: '#34495e'}}>Lý do xuất:</b> 
                                            <select className="tool-select" style={{marginTop: '5px'}} value={transForm.reason} onChange={e => setTransForm({...transForm, reason: e.target.value})}>
                                                <option value="Bán hàng">Bán hàng</option><option value="Xuất sử dụng">Xuất sử dụng (Canh tác)</option><option value="Khác">Khác</option>
                                            </select>
                                        </div>
                                        {transForm.reason === 'Khác' && (<div><b style={{color: '#34495e'}}>Ghi chú lý do:</b> <input className="tool-input" style={{marginTop: '5px'}} value={transForm.note} onChange={e => setTransForm({...transForm, note: e.target.value})} required/></div>)}
                                        
                                        {transForm.reason === 'Bán hàng' && (
                                            <div style={{gridColumn: 'span 2', display: 'flex', flexDirection: 'column', gap: '10px', background: '#eafaf1', padding: '10px', borderRadius: '6px', border: '1px dashed #27ae60'}}>
                                                <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px'}}>
                                                    <label><b>Thuế VAT Bán ra (%):</b>
                                                        <input type="number" step="0.1" min="0" className="tool-input" style={{marginTop: '5px'}} value={transForm.vatRate} onChange={e => setTransForm({...transForm, vatRate: e.target.value})} placeholder="Vd: 8" />
                                                    </label>
                                                    <label><b>Khách hàng (Xã Viên):</b>
                                                ...
                                                {transForm.memberPhone && (
                                                    <label style={{display: 'flex', alignItems: 'center', cursor: 'pointer', background: '#fad390', padding: '8px', borderRadius: '4px'}}>
                                                        ...
                                                        <b style={{color: '#d35400'}}>Cho phép mua chịu (Ghi nợ Xã viên)</b>
                                                    </label>
                                                )}
                                                
                                                {!transForm.isCredit && (
                                                    <label style={{display:'block', marginTop: '5px'}}>
                                                        <b style={{color: '#27ae60'}}>Hình thức Thu tiền vào Quỹ:</b>
                                                        <select className="tool-select" style={{marginTop: '5px'}} value={transForm.paymentMethod} onChange={e => setTransForm({...transForm, paymentMethod: e.target.value})}>
                                                            <option value="Tiền mặt">Tiền mặt</option><option value="Chuyển khoản">Chuyển khoản</option>
                                                        </select>
                                                    </label>
                            ...
                            {transForm.products.map((prod, index) => (
                                <div className="product-row" key={index}>
                                    ...
                                    <input type="number" className="tool-input" placeholder={transForm.type === 'Nhập' ? "Đơn giá Gốc" : "Đơn giá xuất"} value={prod.unitPrice} onChange={(e) => handleProductRowChange(index, 'unitPrice', e.target.value)} />
                            ...
                            {/* HIỂN THỊ TỔNG TIỀN ĐÃ CÓ VAT */}
                            {transForm.type === 'Nhập' ? (
                                <div style={{textAlign: 'right', marginTop: '15px', padding: '10px', background: '#fdfefe', borderRadius: '8px', border: '1px solid #e0e0e0'}}>
                                    <div style={{color: '#7f8c8d'}}>Tiền hàng gốc: {new Intl.NumberFormat('vi-VN').format(calc.subTotal)} đ</div>
                                    <div style={{color: '#e74c3c'}}>+ VAT ({transForm.vatRate || 0}%): {new Intl.NumberFormat('vi-VN').format(calc.vatValue)} đ</div>
                                    <h3 style={{margin: '10px 0 0 0', color: '#c0392b'}}>TỔNG CHI: {new Intl.NumberFormat('vi-VN').format(calc.totalAmount)} đ</h3>
                                </div>
                            ) : (
                                transForm.reason === 'Bán hàng' && (
                                    <div style={{textAlign: 'right', marginTop: '15px', padding: '10px', background: '#fdfefe', borderRadius: '8px', border: '1px solid #e0e0e0'}}>
                                        <div style={{color: '#7f8c8d'}}>Tiền hàng gốc: {new Intl.NumberFormat('vi-VN').format(calc.subTotal)} đ</div>
                                        <div style={{color: '#e74c3c'}}>+ VAT ({transForm.vatRate || 0}%): {new Intl.NumberFormat('vi-VN').format(calc.vatValue)} đ</div>
                                        <h3 style={{margin: '10px 0 0 0', color: transForm.isCredit ? '#d35400' : '#27ae60'}}>
                                            TỔNG {transForm.isCredit ? 'GHI NỢ' : 'THU'}: {new Intl.NumberFormat('vi-VN').format(calc.totalAmount)} đ
                                        </h3>
                                    </div>
                                )
                            )}
            ...
            {/* MODAL 3: XEM CHI TIẾT DANH MỤC HÀNG HÓA (READ-ONLY) */}
            {isViewItemModalOpen && viewItemData && (
                <div className="modal-overlay">
                    <div className="modal-content" style={{ maxWidth: '500px' }}>
                        <div className="modal-header" style={{ background: '#34495e', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 20px' }}>
                            <h3 style={{margin: 0}}>👁️ Chi tiết Hàng hóa</h3>
                            <button onClick={() => setIsViewItemModalOpen(false)} style={{border: 'none', background: 'none', fontSize: '20px', cursor: 'pointer', color: 'white'}}>✕</button>
                        </div>
                        <div className="modal-body">
                            <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Tên mặt hàng:</small>{viewItemData.itemName}</div>
                            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px'}}>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Phân loại:</small>{viewItemData.category}</div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Chất lượng:</small>{viewItemData.quality || '-'}</div>
                            </div>
                            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px'}}>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Số lượng tồn:</small><span style={{color: '#27ae60', fontSize: '18px'}}>{viewItemData.quantity} {viewItemData.unit}</span></div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Đơn giá gốc:</small><span style={{color: '#e74c3c', fontSize: '18px'}}>{new Intl.NumberFormat('vi-VN').format(viewItemData.unitPrice || 0)} đ</span></div>
                            </div>
                            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px'}}>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Số Lô hàng:</small>{viewItemData.batchNumber || '-'}</div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Hạn sử dụng:</small>{viewItemData.expiryDate || '-'}</div>
                            </div>
                            <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Nhà cung cấp:</small>{viewItemData.supplier || '-'}</div>
                            <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Lần cập nhật cuối:</small>{new Date(viewItemData.lastUpdated).toLocaleString('vi-VN')}</div>
                        </div>
                        <div className="modal-footer"><button className="btn btn-outline" onClick={() => setIsViewItemModalOpen(false)}>Đóng</button></div>
                    </div>
                </div>
            )}

            {/* MODAL 4: XEM CHI TIẾT LỊCH SỬ GIAO DỊCH (READ-ONLY) */}
            {isViewTransModalOpen && viewTransData && (
                <div className="modal-overlay">
                    <div className="modal-content" style={{ maxWidth: '600px' }}>
                        <div className="modal-header" style={{ background: viewTransData.type === 'Nhập' ? '#27ae60' : '#e74c3c', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 20px' }}>
                            <h3 style={{margin: 0}}>👁️ Chi tiết Phiếu {viewTransData.type.toUpperCase()}</h3>
                            <button onClick={() => setIsViewTransModalOpen(false)} style={{border: 'none', background: 'none', fontSize: '20px', cursor: 'pointer', color: 'white'}}>✕</button>
                        </div>
                        <div className="modal-body">
                            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px'}}>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Mã Phiếu:</small>{viewTransData.ticketCode}</div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Ngày tạo:</small>{viewTransData.date}</div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Người lập phiếu:</small>{viewTransData.creator}</div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Đối tác (NCC/Khách):</small>{viewTransData.supplier || (viewTransData.memberPhone ? `SĐT: ${viewTransData.memberPhone}` : 'Khách lẻ ngoài HTX')}</div>
                            </div>

                            <h4 style={{ borderBottom: '2px solid #ccc', paddingBottom: '5px', marginTop: '15px' }}>Chi tiết Hàng hóa</h4>
                            <div className="read-only-field">
                                <b style={{fontSize: '16px', color: '#2c3e50'}}>{viewTransData.Inventory?.itemName}</b>
                                <div style={{marginTop: '5px'}}>Số lượng: <b style={{color: viewTransData.type === 'Nhập' ? '#27ae60' : '#e74c3c'}}>{viewTransData.quantity} {viewTransData.Inventory?.unit}</b></div>
                                <div style={{marginTop: '5px'}}>Đơn giá gốc: {new Intl.NumberFormat('vi-VN').format(viewTransData.unitPrice || 0)} đ</div>
                            </div>

                            <h4 style={{ borderBottom: '2px solid #ccc', paddingBottom: '5px', marginTop: '15px' }}>Thông tin Kế toán & Thanh toán</h4>
                            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px'}}>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Thuế VAT áp dụng:</small>{viewTransData.vatRate || 0}%</div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>TỔNG TIỀN SAU THUẾ:</small><b style={{color: '#c0392b', fontSize: '18px'}}>{new Intl.NumberFormat('vi-VN').format(viewTransData.totalAmount || 0)} đ</b></div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Trạng thái thanh toán:</small>
                                    <span style={{color: viewTransData.isCredit || viewTransData.paymentStatus?.includes('nợ') ? '#e67e22' : '#27ae60'}}>{viewTransData.paymentStatus || (viewTransData.isCredit ? 'Ghi nợ' : 'Đã thanh toán')}</span>
                                </div>
                                <div className="read-only-field"><small style={{color: '#7f8c8d', display: 'block'}}>Nguồn tiền / Hình thức:</small>{viewTransData.paymentMethod || '-'}</div>
                            </div>
                            
                            {viewTransData.note && (
                                <div className="read-only-field" style={{marginTop: '10px'}}><small style={{color: '#7f8c8d', display: 'block'}}>Ghi chú:</small>{viewTransData.note}</div>
                            )}
                        </div>
                        <div className="modal-footer"><button className="btn btn-outline" onClick={() => setIsViewTransModalOpen(false)}>Đóng</button></div>
                    </div>
                </div>
            )}
        </div>
